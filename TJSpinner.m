...
//    CGFloat startAngle = RADIANS_TO_DEGREES(angle);
    ...
    CGPoint innerArcStartPoint= CGPointMake(self.innerRadius * cos(DEGREES_RADIANS(startAngle)) + centerPoint.x, self.innerRadius * sin(DEGREES_RADIANS(startAngle)) + centerPoint.y);
    
    
    CGPoint outerArcEndPoint = CGPointMake(self.outerRadius * cos(DEGREES_RADIANS(endAngle)) + centerPoint.x, self.outerRadius * sin(DEGREES_RADIANS(endAngle)) + centerPoint.y);
    
    CGContextAddArc(context, centerPoint.x, centerPoint.y, self.innerRadius, DEGREES_RADIANS(startAngle),DEGREES_RADIANS(endAngle), 0);
    
    CGContextAddLineToPoint(context, outerArcEndPoint.x, outerArcEndPoint.y);
    
    CGContextAddArc(context, centerPoint.x, centerPoint.y, self.outerRadius, DEGREES_RADIANS(endAngle),DEGREES_RADIANS(startAngle), 1);
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=0; i<self.numberOfStrokes; i++) 
    {
        ...
        double angle = DEGREES_RADIANS(angleInDeg);
...
{    
    ...
    UIBezierPath *arc = [UIBezierPath bezierPathWithArcCenter:arcCenter radius:arcRadius startAngle:DEGREES_RADIANS(0) endAngle:DEGREES_RADIANS(360) clockwise:NO];
    ...
    for(float i=0;i<numberOfSteps;i++)
    {
        ...
        CGContextAddArc(context, centerPoint.x, centerPoint.y, self.radius+(self.thickness/2.0), DEGREES_RADIANS(arStartAngle), DEGREES_RADIANS(arcEndAngle), 0);
...
- (void)drawFansInContext:(CGContextRef)context
{
    ...
    for (int i=0; i<fanCount; i++)
    {
        ...
        if ((i%2)==0) 
        {
            ...
            CGFloat startAngleInRad = DEGREES_RADIANS(startAngle);
            CGFloat endAngleInRad = DEGREES_RADIANS(endAngle);
            ...
            CGFloat startArcAngle = startAngleInRad+DEGREES_RADIANS(arcAngle);
            ...
            CGFloat endArcAngle = endAngleInRad+DEGREES_RADIANS(arcAngle);
...
- (void) drawGlossyEffectInContext:(CGContextRef)context
{
   ...
   //White color is used to reflect the glossy effect. Only alpha value need to be changed around the corners.
    ...
    CGContextAddArc(context, centerPoint.x, centerPoint.y, self.radius-1, DEGREES_RADIANS(190), DEGREES_RADIANS(350), 0);
...
- (void) drawEdgeCirceInContext:(CGContextRef)context
{
    ...
    CGPoint centerPoint = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
    CGContextAddArc(context, centerPoint.x, centerPoint.y, self.radius, DEGREES_RADIANS(0), DEGREES_RADIANS(360), 0);
...
- (void) drawShadowEffectInContext:(CGContextRef)context
{
   ...
   //Draw the shadow effect to the rainbow circles
    ...
    CGContextAddArc(context, centerPoint.x, centerPoint.y, self.radius, DEGREES_RADIANS(0), DEGREES_RADIANS(360), 1);
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int index=0; index<_segmets; index++) 
    {
        ...
        [self drawSegmentForIndex:index startAngle:DEGREES_RADIANS(startAngle) endAngle:DEGREES_RADIANS(angle) context:context];
    }
